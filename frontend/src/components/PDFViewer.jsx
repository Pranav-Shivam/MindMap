...
function PDFViewer({ docId, currentPage, pages, onPageChange }) {
  ...
  useEffect(() => {
    const loadPreviews = async () => {
      ...
      for (const page of pages) {
        try {
          ...
          urls[page.page_no] = URL.createObjectURL(blob);
        } catch (error) {
    ...
    // Cleanup URLs when component unmounts or pages change
    return () => {
  ...
  return (
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Thumbnails */}
        <div className="w-32 overflow-y-auto border-r bg-background p-2">
          {pages.map((page) => (
            <div
              key={page.page_no}
              onClick={() => onPageChange(page.page_no)}
              className={`mb-2 cursor-pointer rounded border-2 p-1 transition-colors ${
                page.page_no === currentPage
                  ? 'border-primary'
