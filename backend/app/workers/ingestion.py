    ...
    try:
        ...
        with PDFProcessor(file_path) as pdf_processor:
            ...
            for page_no in range(page_count):
                ...
                try:
                    # Extract all content: text, tables, and images
                    content = pdf_processor.extract_all_page_content(
                        page_no, 
                    ...
                    pdf_processor.generate_page_preview(page_no, preview_path)
                    
                    # Combine all extracted content into a comprehensive text representation
                    combined_content = _combine_page_content(content)
                    
                    # Use LLM vision to extract and rewrite all content from the page image
                    # This ensures we capture everything including complex layouts, tables, and images
                    ...
                    # Use rewritten text as the final text
                    ...
                    if chunks:
                        ...
                        # Prepare chunk data for vector DB
                        chunk_data = []
                        ...
                        vector_client.upsert_chunks(collection_name, chunk_data, embeddings)
                        logger.debug(f"Upserted {len(chunks)} chunks for page {page_no}")
                    
                    # Generate page summary and key terms
                    summary = ""
                    key_terms = []
                    ...
                    # Create page document in CouchDB
                    page_doc = {
...
"""
        ...
        full_response = ""
        async for token in llm_client.stream_chat(messages, temperature=0.3, max_tokens=500):
            full_response += token
